...
export type GltfOptions = {
  ...
  basePath?: string;
  processUri?: Function;
  processAnimationExtras?: Function;
  processMaterialExtras?: Function;
  processGlobalExtras?: Function;
};
...
export type Resources = {
  ...
  processAnimationExtras: Function; // undefined
  processMaterialExtras: Function; // undefined
  processGlobalExtras?: Function;
...
export type GltfNode = {
  name: string; // Bip001_Bip001 GableBack03
  rotation: number[]; // [-0.0004641080158762634, -0.005688844248652458, 0.0661124438047409, 0....
  scale: number[]; // [0.9999999403953552, 1.0000001192092896, 1.0000001192092896]
...
function getAccessorData(gltf: Gltf, accessor: Accessor, buffers: ArrayBuffer[]) {
...
export type GltfSampler = {
  input: number; // 651
  interpolation: 'CUBICSPLINE' | 'CUBIC' | 'STEP' | 'LINEAR';
  output: number; // 652
};

export type GltfAnimation = {
  channels: GltfAnimationChannel[]; // [[object Object], [object Object], [object Object], [object Object], [...
  name: string; // die
  samplers: GltfSampler[]; // [[object Object], [object Object], [object Object], [object Object], [...
};

// Specification:
//   https://github.com/KhronosGroup/glTF/tree/master/specification/2.0#animation
function translateAnimation(data: GltfAnimation, resources: Resources) {
...
function translateNode(data: GltfNode, resources: Resources) {
  ...
  if (data.hasOwnProperty('camera')) {
    console.log('translateNode> camera (disabled)', data.camera);
    /*
    var gltf = resources.gltf;
...
function loadBuffers(resources: Resources, success: CallableFunction) {
...
function parse(property: string, translator: CallableFunction, resources: Resources, success?: CallableFunction) {
  if (success) {
...
function buildHierarchy(resources: Resources) {
  var gltf = resources.gltf;
...
function createModel(resources: Resources) {
  var gltf = resources.gltf;

  var meshInstances: pc.MeshInstance[] = [];
  var skinInstances: pc.SkinInstance[] = [];
  var morphInstances: pc.MorphInstance[] = [];
...
function loadGltf(gltf: Gltf, device: pc.GraphicsDevice, done: CallableFunction, options?: GltfOptions) {
...
function loadGlb(glb: ArrayBuffer, device: pc.GraphicsDevice, done: any, options?: GltfOptions) {
